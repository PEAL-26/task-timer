...
interface Props {
    onEdit?(tarefaId: string | undefined, mostrar: boolean): void;
}

export default function TabelaTarefas(props: Props) {
    const { onEdit } = props;
        ...
        {tarefas && tarefas?.length > 0 ?
            < table className="flex-1 flex flex-col " >
                ...
                <tbody className="flex-grow bg-black-light rounded-md pb-2">
                    {tarefas?.map(({ id, titulo, projecto, dataConclusao }) => (
                        <tr key={id} className='flex w-full border-b border-gray/20 hover:bg-black/10'>
                            <td
                                className="w-[48%] text-left py-2 px-4 flex items-center"
                                onClick={() => {
                                    onEdit && onEdit(id, true);
                                }}
                            >
                                {<TarefaIcon tarefaId={id} />}
                            </td>
